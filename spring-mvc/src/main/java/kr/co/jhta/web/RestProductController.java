 ...
 * 			* 클라이언트에서 서버로 요청메세지를 보낼 때 반드시 요청메세지의 body부에 포함되는 데이터의 컨텐츠 타입을 지정해야한다.
 * 			* 프로젝트에 지정된 컨텐츠 타입의 값을 자바객체로 변환하는 라이브러리가 포함되어있어야한다.
 * 
 * 		- @ResponseBody는 요청핸들러 메소드가 반환하는 객체를 응답메세지의 body부에 담는다.
 * 			public @ResponseBody List<Product> getAllProducts() { ... }
 * 			* 클라이언트는 서버로부터 받은 응답데이터의 body부에 포함되는 데이터의 컨텐츠 타입을 지정해야한다.
 * 			* 프로젝트에 자바객체를 지정된 컨텐츠 타입의 값으로 변환하는 라이브러리가 포함되어있어야한다.
 */
@RestController
@RequestMapping("/rest")
public class RestProductController {
	...
	public Product product(@PathVariable("no") long productNo) {
